...
def salary(request):
    ...
    if request.method == 'POST':
        ...
        for employee_id, records in employee_attendance.items():
            ...
            employee_payroll[employee_id]['semimonthly_rate']= payroll.half_salary
...
def payroll(request):
    if request.method == 'POST':
        ...
        selected_year = int(selected_year)
        selected_month = int(selected_month)
        covered_year = selected_year
        covered_month = selected_month
        start_date = None
        end_date = None
        # print(f"{month_half=}")
        day = ""
        ...
        if month_half == '1':
            ...
            end_date = datetime.datetime(selected_year, selected_month, 15)
            day = "1-15"
            date_payout = f"{pref}{covered_month}/20/{covered_year}"
        elif month_half == '2':
            start_date = datetime.datetime(selected_year, selected_month, 16)
            if selected_month == 12:
                selected_year = selected_year + 1
            ...
            day = "16-"+ str(last_day)
            print(f"{pref=}")
            date_payout = f"{pref}{covered_month+1}/05/{covered_year}"
 
        date_covered = months[covered_month-1] + " " + day + ", " + str(covered_year)
        print(f"{date_covered=}")
        ...
        # for rec in records:
        #     rec['date_covered'] = date_covered
        #     rec['date_payout'] = date_payout
        ...
        # }        
        print(records)
        return JsonResponse({"records":records, "date_info":date_info}, safe = False)
    context = {
        'form': FilterSalaryForm(),
    }
...
def fetch_attendances(request):
    records = list(Attendances.objects.select_related('employee').order_by('-timestamp').values('timestamp', 'overtime', 'employee__dv_name', 'employee__dv_user_id', 'employee_id').filter(employee__is_registered = True))
    ...
    for date, logs in dates.items():
        for x, log in logs.items():
            ...
            time_in = sorted_log[0]['timestamp'].time()
            time_out = sorted_log[-1]['timestamp'].time()
            overtime_stat = sorted_log[-1]['overtime']
            ...
            is_late = False
            is_overtime = False
            if time_in is not None and time_in > datetime.datetime.strptime('08:00:00', '%H:%M:%S').time():
                is_late = True
            if time_out is not None and time_out > datetime.datetime.strptime('17:00:00', '%H:%M:%S').time() and overtime_stat == 1:
                is_overtime = True
            # else:
            #     is_overtime = False
